...
class Hunyuan3DDiTPipeline:
    ...
    def from_single_file(
        ...
        **kwargs,
        ...
        if use_safetensors:
            print('Cargando pesos (safetensors)...')
            ...
            print('Cargando pesos (torch)...')
            ckpt = torch.load(ckpt_path, map_location='cpu', weights_only=True)
        print('Instanciando modelo principal...')
        model = instantiate_from_config(config['model'])
        print('Cargando pesos del modelo principal...')
        model.load_state_dict(ckpt['model'])
        print('Instanciando VAE...')
        vae = instantiate_from_config(config['vae'])
        print('Cargando pesos del VAE...')
        vae.load_state_dict(ckpt['vae'], strict=False)
        print('Instanciando conditioner...')
        conditioner = instantiate_from_config(config['conditioner'])
        if 'conditioner' in ckpt:
            print('Cargando pesos del conditioner...')
            conditioner.load_state_dict(ckpt['conditioner'])
        print('Instanciando image_processor...')
        image_processor = instantiate_from_config(config['image_processor'])
        print('Instanciando scheduler...')
        ...
        print('Finalizando inicialización del pipeline...')
        return cls(
            **model_kwargs
        )
    ...
    def enable_model_cpu_offload(self, gpu_id: Optional[int] = None, device: Union[torch.device, str] = "cpu"):
        r"""
...
class Hunyuan3DDiTFlowMatchingPipeline(Hunyuan3DDiTPipeline):
    """
    Pipeline para generación de modelos 3D únicamente con geometría (sin texturas).
    """
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        Genera un modelo 3D solo con geometría (sin texturas).
        """
        callback = kwargs.pop("callback", None)
        callback_steps = kwargs.pop("callback_steps", None)
