...
# --- INICIO: Endpoint de generación de imagen desde texto (Stable Diffusion) ---
def get_sd_pipeline():
    # Carga perezosa para no ralentizar el arranque
    from diffusers import StableDiffusionPipeline
    import torch
    if not hasattr(get_sd_pipeline, 'pipe'):
        get_sd_pipeline.pipe = StableDiffusionPipeline.from_pretrained(
            'runwayml/stable-diffusion-v1-5',
            torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32
        )
        get_sd_pipeline.pipe = get_sd_pipeline.pipe.to('cuda' if torch.cuda.is_available() else 'cpu')
    return get_sd_pipeline.pipe

@app.post("/generate-image")
async def generate_image(prompt: str = Form(...)):
    """Genera una imagen desde un prompt de texto usando Stable Diffusion."""
    pipe = get_sd_pipeline()
    image = pipe(prompt).images[0]
    output_path = os.path.join(SAVE_DIR, f"generated_{uuid.uuid4().hex[:8]}.png")
    image.save(output_path)
    return FileResponse(output_path, media_type="image/png")
# --- FIN: Endpoint de generación de imagen desde texto ---
